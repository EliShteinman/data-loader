...
REM יצירת תג ייחודי מבוסס חותמת זמן
FOR /f "tokens=1-4 delims=/ " %%a in ("%date%") do ( FOR /f "tokens=1-3 delims=:" %%e in ("%time%") do ( SET IMAGE_TAG=%%c%%b%%a-%%e%%f%%g ) )
...
REM הרצת הבנייה מתיקיית השורש של הפרויקט (..), שם נמצא ה-Dockerfile
...
powershell -Command "(Get-Content ..\infrastructure\k8s\05-fastapi-deployment.yaml) -replace 'YOUR_DOCKERHUB_USERNAME', '%DOCKERHUB_USERNAME%' -replace ':latest', ':%IMAGE_TAG%' | oc apply -f -"
