...
REM This ensures that all relative paths (like ..\infrastructure) work correctly.
cd /d "%~dp0"
...
FOR /F "tokens=*" %%g IN ('oc get pod -l app=mysql -o jsonpath="{.items[0].metadata.name}"') do (SET MYSQL_POD=%%g)
...
FOR /F "tokens=*" %%g IN ('oc get secret mysql-credentials -o jsonpath="{.data.MYSQL_ROOT_PASSWORD}" ^| base64 --decode') do (SET MYSQL_PASSWORD=%%g)
