@echo off
REM --- Step 1 & 2: Setup Project and MySQL ---
echo [INFO] Setting up OpenShift Project and MySQL...
oc new-project data-loader-app || true
...
set "MYSQL_POD=$(oc get pod -l app=mysql -o jsonpath='{.items[0].metadata.name}')"
...
echo [INFO] Executing SQL scripts...
set "MYSQL_PASSWORD=$(oc get secret mysql-credentials -o jsonpath='{.data.MYSQL_ROOT_PASSWORD}' | base64 --decode)"
oc exec %MYSQL_POD% -- mysql -u root -p"%MYSQL_PASSWORD%" mydatabase < create_data.sql
oc exec %MYSQL_POD% -- mysql -u root -p"%MYSQL_PASSWORD%" mydatabase < insert_data.sql
